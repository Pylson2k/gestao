...
export default function PagamentosClientPage() {
  const searchParams = useSearchParams()
  const { payments, addPayment, updatePayment, deletePayment, isLoading, refreshPayments, getTotalPaidByQuoteId } =
    usePayments()
  ...
  // Suporte a link direto: /financeiro/pagamentos?quoteId=xxx ou ?quoteId=xxx&openDialog=1
  useEffect(() => {
  ...
  const availableQuotes = useMemo(() => {
    return quotes.filter(
      (q) => q.status === 'approved' || q.status === 'in_progress' || q.status === 'completed'
    )
  }, [quotes])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (quote) {
      ...
      if (newTotalPaid > quote.total) {
        ...
        alert(
          `Valor excede o total do orçamento. Total: R$ ${quote.total.toFixed(2)}\nJá pago: R$ ${totalPaid.toFixed(
            2
  ...
  const handleWhatsAppPaymentReceipt = (payment: Payment) => {
    ...
    try {
      ...
      if (!digits || digits.length < 10) {
        alert('Cadastre um telefone válido com DDD no cliente para abrir o WhatsApp (ex.: 11999998888).')
        return
      }
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          ...
                          <Badge className={cn('text-xs', paymentMethodColors[payment.paymentMethod])}>
                        ...
                        {payment.observations && (
                          <p className="text-sm text-muted-foreground mt-2">{payment.observations}</p>
                        )}
                      </div>
                ...
                >
                  ...
                  <SelectContent>
                    {availableQuotes.map((quote) => {
                      ...
                      return (
                        <SelectItem key={quote.id} value={quote.id}>
                          {quote.number} - {quote.client.name} (Restante:{' '}
                          {remaining.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })})
                        </SelectItem>
                      )
                    })}
                  ...
                  (() => {
                    ...
                    if (quote) {
                      const totalPaid = payments
                        .filter(
                          (p) =>
                            p.quoteId === formData.quoteId && (!editingPayment || p.id !== editingPayment.id)
                        )
                        .reduce((sum, p) => sum + p.amount, 0)
                      const remaining = quote.total - totalPaid
                      ...
                      return (
                        <p className="text-xs text-muted-foreground">
                          Total do orçamento:{' '}
                          {quote.total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} | Já pago:{' '}
                          {totalPaid.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} | Restante:{' '}
                          {remaining.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                          {amount > remaining && (
                            <span className="text-destructive block mt-1">⚠️ Valor excede o restante!</span>
                          )}
                        </p>
                      )
                    }

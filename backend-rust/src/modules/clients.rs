...
fn require_user(headers: &HeaderMap) -> Result<&str, ApiError> {
    headers
        .get("x-user-id")
...
/// Resolve o id real do usuário no banco (legado mapeia sessão "1" -> username 'gustavo').
...
struct AuditEntry<'a> {
    ...
    new_value: Option<&'a serde_json::Value>,
}

async fn write_audit_log(pool: &PgPool, entry: AuditEntry<'_>, headers: &HeaderMap) {
